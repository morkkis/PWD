...
export class CardsView extends React.Component<CardsViewProps, CardsViewState> {
  ...
  private cellRenderer(cardsInRaw: number, data: GridCellProps) {
    ...
    return (
      <AutoSizer>
        {(data: Size) => {
          ...
          return <Grid {...this.getGridProps(data.width, data.height, isScrolling)} />
        }}
      </AutoSizer>
    );
  };

  private getGridProps(width: number, height: number, isScrolling: boolean) {
    const cardsInRaw = this.calcCardsInRaw();
    const metadata = this.state.metadata;
    const catList: ICat[] = this.state.catList;
    const rowCount =  Math.ceil(catList.length / cardsInRaw);
    return {
      cellRenderer: this.cellRenderer.bind(this, cardsInRaw),
